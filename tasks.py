...
def coverage(c):
    ...
    """
    c.run("coverage run manage.py test gn_places")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/acdh-django-geonames.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ gn_places")
...
def lint(c):
    ...
    """
    c.run("flake8 acdh-django-geonames tests")
